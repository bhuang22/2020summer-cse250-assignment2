...
namespace ubcse{
    ...
    public:
         ...
         *
         *	For example: vector<char> char_vector(5,'b'), wi0ll create a vector
         *	['b','b','b','b','b'].
        ...
        vector<T>(const int n, const T &value){
            ...
            for(int i=0; i< n; i++){
                if(current_capacity==num_items) vector<T>::reserve(2*current_capacity);
                data_array[i]=value;
                num_items++;
         ...
         *
         */
        vector<T>(const vector<T>& other): num_items(other.num_items),
        ...
        vector<T>(T& start, T& end){
         ...
         /*       num_items=0;
            ...
            */
            ...
            T* ptr =&start;
            //cout<<"ptr:"<<*(ptr+1);
            for(T i=0;i<=current_capacity;i++){
                if(*(ptr+i)>end) break;
                if(current_capacity==num_items) vector<T>::reserve(2*current_capacity);
                data_array[i]=*(ptr+i);
                num_items++;
            ...
            // implement rest of the functionalities
    ...
    void vector<T>::concat(const vector<T>& other){
        ...
        for(int i=0;i<other.num_items;i++) {
            data_array[num_items] = other.data_array[i];
            num_items++;
    ...
    void vector<T>::shrink_to_fit(){
        ...
        if(data_array==NULL)current_capacity = INITIAL_CAPACITY;
        else{
